...
const TimeSeriesNetwork = (props: TimeSeriesNetworkProps) => {
  ...
  useEffect(() => {
    d3.select(`#time_series_minutes_${tab} > svg`).remove();
    const svg = d3
    ...
    const maxYAxisFirst =
      d3.max(dataTimeSeriesFirst, (d: TimeSeriesNetworkDataProps) => +d.average) || 150;
    const maxYAxisSecond =
      d3.max(dataTimeSeriesSecond, (d: TimeSeriesNetworkDataProps) => +d.average) || 150;
    ...
    svg
      ...
      .on('mouseover', function (event, d) {
        const tooltip = d3
          .select(`#time_series_minutes_${tab}`)
          .append('div')
        ...
        tooltip
          .html(
            `Timestamp: ${new Date(d.timeStamp).toUTCString()}<br>
                Average: ${Math.round(d.average)} MB<br>`,
          )
          .style('opacity', 0.9)
      ...
      .on('mouseout', function () {
        ...
        d3.select(`#time_series_minutes_${tab} > div.tooltip-all-node`).remove();
      });
    ...
    svg
      ...
      .on('mouseout', function () {
        d3.select(this).attr('r', 3);
        d3.select(`#time_series_minutes_${tab} > div.tooltip-selected-node`).remove();
      });
    ...
    <>
      <Box sx={{ width: width, height: height * 1.25, background: '#FFFFFF' }}>
        ...
        <Group position="center" mt="md">
          ...
          <Text weight={650}>{tab == 'network' ? 'Network In' : 'Disk Read Operation'}</Text>
          <Box sx={{ width: '3rem', height: '1.5rem', background: '#A8DBF8 ' }}></Box>
          <Text weight={650}>{tab == 'network' ? 'Network Out' : 'Disk Write Operation'}</Text>
        </Group>
        <Box id={`time_series_minutes_${tab}`} ref={svgRef}></Box>
      </Box>
    </>
  );
};
