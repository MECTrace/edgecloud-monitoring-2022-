...
const checkStatusOfNode = (communicationEvent: ISocketEvent[], id: string) => {
...
export const createNodesAndEdges = (
  ...
  listDevice &&
    listDevice.map((item, index) => {
      ...
      const degrees = index * (360 / (listDevice.length - 1));
      const radians = degrees * (Math.PI / 180);
      const x = 250 * Math.cos(radians) + center.x;
      const y = 250 * Math.sin(radians) + center.y;

      const position = typeNode === 0 ? center : { x, y };
      const type = typeNode === 0 ? 'default' : 'customNode';
      const status = communicationEvent && checkStatusOfNode(communicationEvent, id);
      nodes.push({
        id,
